...
export function recommendItems(currentItems, baskets, { limit = 10, minSupport = 0.01 } = {}) {
  const itemsSet = new Set(currentItems.map(safe));
  const total = baskets.length || 1;
  const supportItem = new Map();
  const supportPair = new Map(); // key: a||b
  ...
  const support = (x) => (supportItem.get(x) || 0) / total;
  const supportXY = (x, y) => {
  ...
  const candidates = new Map(); // item -> metrics
...
export function buildTips(currentItems, baskets) {
  ...
  const complements = [
    ...recs.map(r => ({ item: r.item, score: r.score, reason: 'Frequente com seus itens' })),
    ...extra
  ]
  ...
  const itemDates = new Map(); // name -> [timestamps]
  for (const b of baskets) {
